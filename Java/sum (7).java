...
public class sum{
	public static void main(String[] args){
		...
		try{
			...
			Scanner myScan = new Scanner(file);String st;
			while (myScan.hasNextLine()){
				...
				int[] result = new int[25];
				int[] resultT = new int[25];
				while(myNumScan.hasNext()){
					int[] integer = new int[25];
					String data = myNumScan.next();
					AtoI(integer, data);
					printA(integer);
					if(myNumScan.hasNext()== true){
						System.out.print(" + ");
					}
					addA(resultT,result,integer);
					//math = math.add(data);
					//printA(result);
				}
				System.out.printf(" = ");
