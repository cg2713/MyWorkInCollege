...
void list::addFront(const int& e) {
    typename list::Node* v = new Node;
    v->elem = e;
    v->next = head;
    head = v;
}
int list::removeFront() {
    typename list::Node* old = head;
    int v = head->elem;
    head = old->next;
    delete old;
    return v;
...
bool list::remove(int v) {
    typename list::Node* p1 = head;
    typename list::Node* p2 = p1;
    if(head->elem == v)
        return removeFront();
    while(p1 != NULL) {
        if(p1->elem == v) {
...
void list::addBack(const int& e){ // if its void dount change to int
    typename list::Node* last = new Node;
    ...
    if (head == NULL) {
        ...
        typename list::Node* temp = new Node;
